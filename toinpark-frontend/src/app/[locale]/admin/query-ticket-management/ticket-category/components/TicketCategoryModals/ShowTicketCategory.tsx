...
import { useGetATicketCategoryQuery } from "@/store/api/tickets-categories/tickets-categories-api";
import ContentLoader from "@/components/feature/loader/ContentLoader";

export default function ShowTicketCategory({
  ticketId,
}: {
  ticketId: TString;
}) {
  ...
  const { data: getATicketCategoryRes, ...getATicketCategoryApiState } =
  ...
  const categoryData = getATicketCategoryRes?.data;
    ...
    <>
      ...
      >
        <RenderData
          ...
          data={categoryData}
          {...getATicketCategoryApiState}
          loadingSkeleton={<ContentLoader />}
        >
          <ScrollArea className="max-h-[60vh] pr-2">
            <div className="space-y-4">
              {/* Name */}
              <InfoCard label="Name" value={categoryData?.name} />
