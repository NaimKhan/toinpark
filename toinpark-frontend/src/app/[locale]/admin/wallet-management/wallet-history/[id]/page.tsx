...
import AdminWalletHistoryTable from "./components/AdminWalletHistoryTable";
import { useParams } from "next/navigation";
import GradientText from "@/components/feature/text/gradientText";

function AdminWalletHistoryPage() {
  const params = useParams();
  const id = params.id as string;
  console.log(id);

  return (
    <div className="space-y-6 md:space-y-10 px-6 xl:px-16 md:px-10 lg:my-10 my-6">
      <div className="flex flex-wrap items-end gap-6 w-full ">
        <div className="space-y-4 flex-1 text-start">
          ...
          <p className="text-default-200 text-lg font-normal  mx-auto">
            View all your TOIN wallet activities and track balance changes over
            time.
          </p>
        </div>
      </div>
      <div className="space-y-8 md:space-y-10">
        <AdminWalletHistoryTable userId={id} />
      </div>
    </div>
  );
}

export default AdminWalletHistoryPage;
